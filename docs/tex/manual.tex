...
\begin{document} % All begin commands must be paired with an end command somewhere
    ...
    \hypertarget{sec:nspanel_flashing}{}
    Prerequisites:
    \begin{itemize}
    ...
    \begin{itemize}
      ...
      \item Enter WiFi name (or select from a list by pressing the "Select from list" button) and password.
      \important{There has been an observed behavior that the NSPanel in the current framework version does not play nicely with WPA3 WiFi security.}
    ...
    \begin{itemize}
      ...
      \item \textbf{Entity update mode} - When changing an entity state on the NSPanel itself, should the panel and assume that the new state we be set correctly (optimistic) or should the panel and manager wait for a state update from the entity before displaying the new state (wait).
    \end{itemize}
    ...
    nspanel/<panel\_name>/r1\_cmd & 1, 0 or 2 & Send a 1, 0 or 2 to turn on, off or toggle relay 1. \\ \hline
    nspanel/<panel\_name>/r1\_state & 1 or 0 & The current state of relay 1. \\ \hline
    nspanel/<panel\_name>/r2\_cmd & 1, 0 or 2 & Send a 1, 0 or 2 to turn on, off or toggle relay 2. \\ \hline
