(define (nspanel-us-tft3 xcf-path)
  (let* (
	...
	(gimp-item-set-visible (car (gimp-image-get-layer-by-name image "tft3")) 1)
...
;FIRST_PAGE FIRST_PAGE FIRST_PAGE FIRST_PAGE FIRST_PAGE FIRST_PAGE FIRST_PAGE FIRST_PAGE FIRST_PAGE 
	...
	(gimp-image-merge-visible-layers newImage 1)
		...
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/58_first_page_ON.bmp" "pictures_tft3/58_first_page_ON.bmp")
		)
	...
	(gimp-image-merge-visible-layers newImage 1)
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/57_first_page_OFF.bmp" "pictures_tft3/57_first_page_OFF.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	(gimp-image-merge-visible-layers newImage 1)
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/59_first_page_scenes_room_off.bmp" "pictures_tft3/59_first_page_scenes_room_off.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	(gimp-image-merge-visible-layers newImage 1)
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/60_first_page_scenes_room_on.bmp" "pictures_tft3/60_first_page_scenes_room_on.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	(gimp-image-merge-visible-layers newImage 1)
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/62_first_page_scenes_all_on.bmp" "pictures_tft3/62_first_page_scenes_all_on.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	(gimp-image-merge-visible-layers newImage 1)
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/61_first_page_scenes_all_off.bmp" "pictures_tft3/61_first_page_scenes_all_off.bmp")
		)
	(gimp-image-delete newImage)
	)
...
;; ENTITY_PAGE_4 ENTITY_PAGE_4 ENTITY_PAGE_4 ENTITY_PAGE_4 ENTITY_PAGE_4 ENTITY_PAGE_4 ENTITY_PAGE_4 
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/67_ENTITY_PAGE_4_empty.bmp" "pictures_tft3/67_ENTITY_PAGE_4_empty.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/68_ENTITY_PAGE_4_press.bmp" "pictures_tft3/68_ENTITY_PAGE_4_press.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/73_scenes_page_loadbar_0.bmp" "pictures_tft3/73_scenes_page_loadbar_0.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/74_scenes_page_loadbar_100.bmp" "pictures_tft3/74_scenes_page_loadbar_100.bmp")
		)
	(gimp-image-delete newImage)
	)
...
;; ENTITY_PAGE_8 ENTITY_PAGE_8 ENTITY_PAGE_8 ENTITY_PAGE_8 ENTITY_PAGE_8 ENTITY_PAGE_8 ENTITY_PAGE_8 ENTITY_PAGE_8  	
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/69_ENTITY_PAGE_8_empty.bmp" "pictures_tft3/69_ENTITY_PAGE_8_empty.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/70_ENTITY_PAGE_8_press.bmp" "pictures_tft3/70_ENTITY_PAGE_8_press.bmp")
		)
	(gimp-image-delete newImage)
	)
...
;; ENTITY_PAGE_12 ENTITY_PAGE_12 ENTITY_PAGE_12 ENTITY_PAGE_12 ENTITY_PAGE_12 ENTITY_PAGE_12 ENTITY_PAGE_12 	
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/71_ENTITY_PAGE_12_empty.bmp" "pictures_tft3/71_ENTITY_PAGE_12_empty.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/72_ENTITY_PAGE_12_press.bmp" "pictures_tft3/72_ENTITY_PAGE_12_press.bmp")
		)
	(gimp-image-delete newImage)
	)
...
;; LIGHTS_PAGE LIGHTS_PAGE LIGHTS_PAGE LIGHTS_PAGE LIGHTS_PAGE LIGHTS_PAGE LIGHTS_PAGE LIGHTS_PAGE LIGHTS_PAGE LIGHTS_PAGE 	
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/75_lights_page_empty.bmp" "pictures_tft3/75_lights_page_empty.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/76_lights_page_brightness_off.bmp" "pictures_tft3/76_lights_page_brightness_off.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/78_lights_page_kelvin_off.bmp" "pictures_tft3/78_lights_page_kelvin_off.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/80_lights_page_color_off.bmp" "pictures_tft3/80_lights_page_color_off.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/77_lights_page_brightness_on.bmp" "pictures_tft3/77_lights_page_brightness_on.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/79_lights_page_kelvin_on.bmp" "pictures_tft3/79_lights_page_kelvin_on.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/81_lights_page_color_on.bmp" "pictures_tft3/81_lights_page_color_on.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/82_lights_page_saturation_off.bmp" "pictures_tft3/82_lights_page_saturation_off.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/83_lights_page_saturation_on.bmp" "pictures_tft3/83_lights_page_saturation_on.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/85_lights_page_corner_empty.bmp" "pictures_tft3/85_lights_page_corner_empty.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/86_lights_page_corner_kelvin.bmp" "pictures_tft3/86_lights_page_corner_kelvin.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/87_lights_page_corner_color.bmp" "pictures_tft3/87_lights_page_corner_color.bmp")
		)
	(gimp-image-delete newImage)
	)
...
;; SCREENSAVER_PAGE SCREENSAVER_PAGE SCREENSAVER_PAGE SCREENSAVER_PAGE SCREENSAVER_PAGE SCREENSAVER_PAGE SCREENSAVER_PAGE 
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/63_screensaver_page.bmp" "pictures_tft3/63_screensaver_page.bmp")
		)
	(gimp-image-delete newImage)
	)
...
;; DROPDOWN_PAGE DROPDOWN_PAGE DROPDOWN_PAGE DROPDOWN_PAGE DROPDOWN_PAGE DROPDOWN_PAGE DROPDOWN_PAGE DROPDOWN_PAGE    	
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/64_dropdown_depress.bmp" "pictures_tft3/64_dropdown_depress.bmp")
		)
	(gimp-image-delete newImage)
	)
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/65_dropdown_press.bmp" "pictures_tft3/65_dropdown_press.bmp")
		)
	(gimp-image-delete newImage)
	)
...
;; BACKGROUND_PAGE BACKGROUND_PAGE BACKGROUND_PAGE BACKGROUND_PAGE BACKGROUND_PAGE BACKGROUND_PAGE BACKGROUND_PAGE   	
	...
	((newImage (car (gimp-image-duplicate image))))
		...
		(let*
		((activeLayer (car (gimp-image-get-active-layer newImage))))
		(gimp-file-save RUN-NONINTERACTIVE image activeLayer "pictures_tft3/66_background.bmp" "pictures_tft3/66_background.bmp")
		)
	(gimp-image-delete newImage)
	)
...
(script-fu-register "nspanel-us-tft3"
  _"Export NSPanel US"
