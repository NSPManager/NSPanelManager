...
#include "switch/homey_switch.hpp"
#include "switch/switch.hpp"
#include "thermostat/home_assistant_thermostat.hpp"
#include "thermostat/homey_thermostat.hpp"
...
void EntityManager::load_rooms()
{
  ...
  EntityManager::_rooms.erase(std::remove_if(EntityManager::_rooms.begin(), EntityManager::_rooms.end(), [&room_ids](auto room)
                                             { return std::find_if(room_ids.begin(), room_ids.end(), [&room](auto id)
                                                                   { return id == room->get_id(); }) == room_ids.end(); }),
...
void EntityManager::load_nspanels()
{
  ...
  EntityManager::_nspanels.erase(std::remove_if(EntityManager::_nspanels.begin(), EntityManager::_nspanels.end(), [&nspanel_ids](auto nspanel)
                                                { return std::find_if(nspanel_ids.begin(), nspanel_ids.end(), [&nspanel](auto id)
                                                                      { return id == nspanel->get_id(); }) == nspanel_ids.end(); }),
...
void EntityManager::load_lights()
{
  ...
  EntityManager::_entities.erase(std::remove_if(EntityManager::_entities.begin(), EntityManager::_entities.end(), [&light_ids](auto entity)
                                                { return entity->get_type() == MQTT_MANAGER_ENTITY_TYPE::LIGHT && std::find_if(light_ids.begin(), light_ids.end(), [&entity](auto id)
                                                                                                                               { return id == entity->get_id(); }) == light_ids.end(); }),
    ...
    else
    {
      ...
      try
      {
        ...
        if (entity_data.contains("controller"))
        {
          ...
          {
            std::shared_ptr<HomeyLight> light = std::shared_ptr<HomeyLight>(new HomeyLight(light_settings.id));
            SPDLOG_INFO("Light {}::{} was found in database but not in config. Creating light.", light->get_id(), light->get_name());
            EntityManager::_entities.push_back(light);
          }
          else
...
void EntityManager::load_buttons()
{
  ...
  EntityManager::_entities.erase(std::remove_if(EntityManager::_entities.begin(), EntityManager::_entities.end(), [&button_ids](auto entity)
                                                { return entity->get_type() == MQTT_MANAGER_ENTITY_TYPE::BUTTON && std::find_if(button_ids.begin(), button_ids.end(), [&entity](auto id)
                                                                                                                                { return id == entity->get_id(); }) == button_ids.end(); }),
                                 EntityManager::_entities.end());
    ...
    else
    {
      ...
      try
      {
        ...
        if (entity_data.contains("controller"))
        {
          ...
          {
            std::shared_ptr<ButtonEntity> button_entity = std::shared_ptr<ButtonEntity>(new HomeyButton(button_settings.id));
            SPDLOG_INFO("Button {}::{} was found in database but not in config. Creating button.", button_entity->get_id(), button_entity->get_name());
            EntityManager::_entities.push_back(button_entity);
          }
          else
...
void EntityManager::load_thermostats()
{
  ...
  EntityManager::_entities.erase(std::remove_if(EntityManager::_entities.begin(), EntityManager::_entities.end(), [&thermostat_ids](auto entity)
                                                { return entity->get_type() == MQTT_MANAGER_ENTITY_TYPE::THERMOSTAT && std::find_if(thermostat_ids.begin(), thermostat_ids.end(), [&entity](auto id)
                                                                                                                                    { return id == entity->get_id(); }) == thermostat_ids.end(); }),
                                 EntityManager::_entities.end());
    ...
    else
    {
      ...
      try
      {
        ...
        if (entity_data.contains("controller"))
        {
          ...
          {
            std::shared_ptr<ThermostatEntity> thermostat_entity = std::shared_ptr<ThermostatEntity>(new HomeyThermostat(thermostat_settings.id));
            SPDLOG_INFO("Thermostat {}::{} was found in database but not in config. Creating thermostat.", thermostat_entity->get_id(), thermostat_entity->get_name());
            EntityManager::_entities.push_back(thermostat_entity);
          }
          else
...
void EntityManager::load_switches()
{
  ...
  EntityManager::_entities.erase(std::remove_if(EntityManager::_entities.begin(), EntityManager::_entities.end(), [&switch_ids](auto entity)
                                                { return entity->get_type() == MQTT_MANAGER_ENTITY_TYPE::SWITCH_ENTITY && std::find_if(switch_ids.begin(), switch_ids.end(), [&entity](auto id)
                                                                                                                                       { return id == entity->get_id(); }) == switch_ids.end(); }),
                                 EntityManager::_entities.end());
    ...
    else
    {
      ...
      try
      {
        ...
        if (entity_data.contains("controller"))
        {
          ...
          {
            std::shared_ptr<HomeySwitch> switch_entity = std::shared_ptr<HomeySwitch>(new HomeySwitch(switch_settings.id));
            SPDLOG_INFO("Switch {}::{} was found in database but not in config. Creating switch.", switch_entity->get_id(), switch_entity->get_name());
            EntityManager::_entities.push_back(switch_entity);
          }
          else
...
void EntityManager::load_scenes()
{
  ...
  EntityManager::_entities.erase(std::remove_if(EntityManager::_entities.begin(), EntityManager::_entities.end(), [&scene_ids](auto entity)
                                                { return entity->get_type() == MQTT_MANAGER_ENTITY_TYPE::SCENE && std::find_if(scene_ids.begin(), scene_ids.end(), [&entity](auto id)
                                                                                                                               { return entity->get_id() == id; }) == scene_ids.end(); }),
                                 EntityManager::_entities.end());
  ...
  for (auto &scene_id : scene_ids)
  {
    ...
    else
    {
      ...
      try
      {
        ...
        else if (scene_settings.scene_type.compare("homey") == 0)
        {
          std::shared_ptr<HomeyScene> scene = std::shared_ptr<HomeyScene>(new HomeyScene(scene_settings.id));
          SPDLOG_INFO("Scene {}::{} was found in database but not in config. Creating scene.", scene->get_id(), scene->get_name());
          EntityManager::_entities.push_back(scene);
        }
        else
...
{
  {
    ...
    EntityManager::_global_room_entities_pages.erase(std::remove_if(EntityManager::_global_room_entities_pages.begin(), EntityManager::_global_room_entities_pages.end(), [&global_page_ids](auto page)
                                                                    { return std::find_if(global_page_ids.begin(), global_page_ids.end(), [&page](auto id)
                                                                                          { return page->get_id() == id; }) == global_page_ids.end(); }),
                                                     EntityManager::_global_room_entities_pages.end());

    // Cause existing NSPanel to reload config or add a new NSPanel if it does not exist.
    for (auto &page_id : global_page_ids)
    {
      auto existing_page = std::find_if(EntityManager::_global_room_entities_pages.begin(), EntityManager::_global_room_entities_pages.end(), [&page_id](auto page)
                                        { return page->get_id() == page_id; });
          ...
          else
          {
            ...
            if (room)
            {
              ...
              bool any_light_entity_on = std::find_if(room_entities.begin(), room_entities.end(), [](auto &entity)
                                           ...
                                           return light && light->get_state(); }) != room_entities.end();
