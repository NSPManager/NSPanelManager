...
void OpenhabManager::reload_config() {
  ...
  {
    std::lock_guard<std::mutex> lock_guard(OpenhabManager::_setting_values_mutex);
    std::string address = MqttManagerConfig::get_setting_with_default<std::string>(MQTT_MANAGER_SETTING::OPENHAB_ADDRESS);
    std::string token = MqttManagerConfig::get_setting_with_default<std::string>(MQTT_MANAGER_SETTING::OPENHAB_TOKEN);
