...
HomeyButton::HomeyButton(uint32_t button_id) : ButtonEntity(button_id)
{
    ...
    {
        SPDLOG_ERROR("No homey_device_id defined for Button {}::{}", this->_id, this->_friendly_name);
        return;
    }

    SPDLOG_DEBUG("Loaded Homey button {}::{}, device ID: {}", this->_id, this->_friendly_name, this->_homey_device_id);
...
{
    SPDLOG_DEBUG("Homey button {}::{} send_state_update_to_controller (trigger button press)", this->_id, this->_friendly_name);
    ...
    {
        SPDLOG_ERROR("Homey address or token not configured for button {}::{}", this->_id, this->_friendly_name);
        return;
    }
    ...
    try
    {
        ...
        SPDLOG_DEBUG("Homey button {}::{} trigger response: {}", this->_id, this->_friendly_name, response);
    }
    catch (const std::exception &e)
    {
        SPDLOG_ERROR("Failed to trigger Homey button {}::{}: {}", this->_id, this->_friendly_name, e.what());
    }
...
void HomeyButton::homey_event_callback(nlohmann::json data)
{
    try
    {
        ...
        SPDLOG_DEBUG("Got event update for Homey button {}::{}.", this->_id, this->_friendly_name);
