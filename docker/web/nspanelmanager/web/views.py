...
def save_panel_settings(request, panel_id: int):
    ...
    if request.POST["screensaver_activation_timeout"].strip():
        set_nspanel_setting_value(
            ...
            request.POST["screensaver_activation_timeout"],
        )
    else:
    ...
    if request.POST["show_screensaver_inside_temperature"] == "global":
        ...
        set_nspanel_setting_value(
            ...
            request.POST["show_screensaver_inside_temperature"],
        )
    ...
    if request.POST["show_screensaver_outside_temperature"] == "global":
        ...
        set_nspanel_setting_value(
            ...
            request.POST["show_screensaver_outside_temperature"],
        )
...
def settings_page(request):
    ...
    data["homey_address"] = get_setting_with_default("homey_address")
    data["homey_token"] = get_setting_with_default("homey_token")
...
def save_settings(request):
    ...
    if "homey_address" in request.POST:
        homey_address = request.POST["homey_address"]
        if homey_address.endswith("/"):
            homey_address = homey_address[:-1]
        set_setting_value(name="homey_address", value=homey_address)

    if "clear_homey_token" in request.POST:
        set_setting_value(name="homey_token", value="")
    elif "homey_token" in request.POST:
        if request.POST["homey_token"] != "":
            set_setting_value(name="homey_token", value=request.POST["homey_token"])
    ...
    set_setting_value(
        name="special_mode_release_time",
        value=request.POST["special_mode_release_time"],
    )

    set_setting_value(
        name="screensaver_activation_timeout",
